...
#   filename:  vector_store_service.json
...
class V1EmbeddingProvider(StrEnum):
    EMBEDDING_PROVIDER_UNSPECIFIED = 'EMBEDDING_PROVIDER_UNSPECIFIED'
    EMBEDDING_PROVIDER_OPENAI = 'EMBEDDING_PROVIDER_OPENAI'
    EMBEDDING_PROVIDER_GOOGLE = 'EMBEDDING_PROVIDER_GOOGLE'
...
class V1VectorStoreSpec(BaseModel):
    apply_uniqueness_index_changes: bool | None = Field(
        None,
    ...
    dimensions: int | None = Field(
        None, description='Dimensions is the number of dimensions to be used.'
    )
    embedding_model: V1EmbeddingModel = Field(
        ..., description='EmbeddingModel is the embedding model of the vector store.'
    )
    embedding_provider: V1EmbeddingProvider = Field(
        ...,
        description='EmbeddingProvider is the embedding provider of the vector store.',
    )
...
class Object(BaseModel):
    ...
    spec: V1VectorStoreSpec | None = Field(
        None, description='The internal specification of the object.'
    )
    tenant_meta: V1TenantMeta | None = Field(
        None, description='VectorStores are associated with a tenant or are global.'
    )
    uuid: str | None = Field(None, description='The UUID of an VectorStore.')


class V1ListVectorStoresResponseList(BaseModel):
    objects: list[Object] | None = None
    response: V1ListResponse | None = None


class V1VectorStore(BaseModel):
    meta: V1Meta = Field(..., description='Metadata required for all objects.')
    propagate: bool | None = Field(
        None,
        description='Vector stores can be propagated and accessed in child namespaces.',
...
class V1ListVectorStoresResponse(BaseModel):
    count_response: V1CountResponse | None = None
    group_response: V1GroupResponse | None = None
    list: V1ListVectorStoresResponseList | None = None


class VectorStoreService(BaseModel):
    v1ListVectorStoresResponse: V1ListVectorStoresResponse | None = None
    v1VectorStore: V1VectorStore | None = None
