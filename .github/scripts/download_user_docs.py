...
This script downloads all documentation pages from the sitemap.xml
...
Examples:
  ...
  python download_user_docs.py \\
    --urls "https://docs.endorlabs.com/getting-started,\\
https://docs.endorlabs.com/api-reference" \\
    --output-dir "external_docs/user-docs" \\
    --timeout 10
        """
    )
    ...
    try:
        ...
        if downloaded_count < len(sitemap_urls):
            print(
                f"Note: {len(sitemap_urls) - downloaded_count} URLs "
                f"were skipped or failed"
            )
