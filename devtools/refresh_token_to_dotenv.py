...
Example::
    ...
    uv run python devtools/refresh_token_to_dotenv.py
    uv run python devtools/refresh_token_to_dotenv.py --env-file .env --timeout 180
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Browser SSO token refresh; writes ENDOR_TOKEN to .env.",
    )
    parser.add_argument(
        "--env-file",
        type=Path,
    ...
    parser.add_argument(
        ...
        help=(
            "API host segment for auth URL (default: endorlabs.com → api.endorlabs.com)"
        ),
    )
    parser.add_argument(
        "--timeout",
