...
downloaded: 2025-10-27 12:57:43
---
...
* `endorctl scan --pr` - You can use the `endorctl scan --pr` command to scan a specific version of your source code for security and operational risks as part of your continuous integration workflows or CI runs. The `endorctl scan --pr` command performs a one-time evaluation of your project, focusing on security and operational risks, rather than providing continuous monitoring. CI runs are shown in the **Scan History** section of each project and are stored for 30 days so that you can analyze and review them on the Endor Labs user interface. See [PR scans](../../scan-with-endorlabs/pr-scans/) for more information.
