...
Update uses `update_mask` + payload (or kwargs via `resource.update(facade, **kwargs)`). Mutable fields are resource-specific; see each resource module and BaseResource.get_mutable_fields() / get_update_kwarg_to_path().
