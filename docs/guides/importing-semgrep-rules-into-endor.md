...
See [authoring-opengrep-rules.md](authoring-opengrep-rules.md) for the
...
```
Author YAML --> Validate with opengrep --> Import with maneuver --> Verify with endorctl
```

### Step 1: Author and validate

Write the rule YAML following the guides in this directory.  Run it
through OpenGrep locally and confirm findings match your expectations.
...
Compare the finding count and affected files against your local OpenGrep
results.
...
The rule YAML is already validated by your local OpenGrep run; the API
parses `spec.yaml` server-side regardless.
...
3. **Finding comparison**: Compare findings:
   - Local OpenGrep finding count should match platform finding count
   - File paths in findings should align
   - Severity levels should match

4. **Example verification** (from our RedactingFilter rule):
   ```
