...
- `client.PackageManager` (list/get/create/update/delete) and `client.SystemConfig` (list/get/update) registry facades.
...
- Executive packet Endor Patches page (`reports.patches` / `05-endor-patches.html`): reach-weighted Available risk, version heat map, impact calculator; CLI `--patches` / `--patches-only`.
...
- Executive packet omits Patches and SAST pages (nav + CSVs) when slices are skipped or unentitled via `EndorLicense`; fail open if the license probe fails.
...
- Executive packet code burndown: **OpenGrep vs AI-SAST** categories (disjoint on `FINDING_TAGS_AI`) share **TP/FP triage facets**; Secrets keeps valid/invalid only.
- SCA FindingLog burndown defaults to **any** reachability (unfiltered counts); reach dropdown keeps RF / PRF / RF∨PRF / unreachable function (dependency-axis facets dropped when RF implies RD).
...
- APIClient log redaction expands credential key coverage and scrubs `token=` query shapes in `_redact_log_data` (defense in depth alongside `RedactingFilter`).
...
- `endor-reports` unified CLI with subcommands for all tenant report workflows: `packet`, `parity`, `duplicates`, `cli-vs-cloud`, `login-count`, `credential-expiry`, `auth-policies`, `ci-endorctl`, `findings-trend`, `prf-analysis`.
- `endorlabs.workflows.reports` layout: `analyze/` → `export/` → `bundles/`, plus portable `parity` helpers comparing packet cubes to gitignored scratch baselines.
- Executive report packet HTML uses Endor black/green branding with bundled logo assets.
- FindingLog multi-series helpers on `endorlabs.workflows.findings.finding_log_trends`: `query_severity_reach_series_cell`, `query_severity_reach_matrix`, `sum_series_cells`, and related rollup utilities (shared by the report packet and chart workflows).
...
- Models aligned to platform OpenAPI / endorctl snapshot 1.7.1080.
...
- Removed ``InitStatus.openapi_path`` alias; use ``platform_openapi_path``.
- Removed ``session_workspace_dir`` and ``workflow_artifacts_root``; use ``default_runs_dir`` / ``workflow_inventory_root``.
- Removed flat ``.endorlabs-context/openapiv2.swagger.json`` reconcile; OpenAPI lives under ``platform/openapi/`` only.
- `endorlabs.workflows.reports.packet.*` import paths are deprecated one release; use `reports.analyze`, `reports.export`, `reports.bundles`, and `reports.schemas` instead. `endor-reports` entry point is `endorlabs.workflows.reports.cli:main`.
