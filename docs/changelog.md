...
- Executive packet Endor Patches page (`reports.patches` / `05-endor-patches.html`): reach-weighted Available risk, version heat map, impact calculator; CLI `--patches` / `--patches-only` (campaign output under `runs/patches-reports/`).
