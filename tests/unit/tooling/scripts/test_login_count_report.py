...
def test_write_login_count_csv_round_trip(tmp_path: Path) -> None:
    ...
    activity = [
        LoginActivityRow(
            identity="john.khotsyphom@cyera.io",
            user_identifiers=(
                "email=john.khotsyphom@cyera.io",
                "id=12345",
            ),
    ...
    assert "john.khotsyphom@cyera.io" in text
    assert ",9" in text
...
def test_build_summary_includes_identity_count() -> None:
    ...
    activity = [
        LoginActivityRow(
            identity="aakash.shinghal@smarsh.com",
            user_identifiers=("email=aakash.shinghal@smarsh.com",),
