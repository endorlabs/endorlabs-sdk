...
from endor_cockpit.resources import findings
from endor_cockpit.resources.findings import Finding
...
class TestFinding:
    ...
    def setup(self):
        ...
        # Get test data
        self.findings = findings.list_findings(self.client, self.namespace)
    ...
    def test_finding_get_list(self):
        ...
        for i, finding in enumerate(findings_list[:10]):  # Show first 10
            print(f"Finding {finding.uuid}: {finding.meta.name}")
    ...
    def test_finding_get_by_uuid(self):
        ...
        retrieved_finding = findings.get_finding(self.client, self.namespace, finding.uuid)
        
        assert retrieved_finding is not None, "Should successfully retrieve finding by UUID"
        assert retrieved_finding.uuid == finding.uuid, "Retrieved finding should match original"
        assert retrieved_finding.meta.name == finding.meta.name, "Finding name should match"
    ...
    def test_finding_patch_tags(self):
        ...
        updated_finding = add_finding_tag(self.client, self.namespace, finding.uuid, test_meta_tag, "meta")
        assert updated_finding is not None, "Should successfully add finding meta tag"
        
        # Verify meta tag was added
        meta_tags_after_add = list_finding_tags(self.client, self.namespace, finding.uuid, "meta")
        assert test_meta_tag in meta_tags_after_add, "Meta tag should be present after add"
        print(f"[SUCCESS] Added meta tag '{test_meta_tag}' to finding")
        
        # Test removing the meta tag
        final_finding = remove_finding_tag(self.client, self.namespace, finding.uuid, test_meta_tag, "meta")
        assert final_finding is not None, "Should successfully remove finding meta tag"
        
        # Verify meta tag was removed
        meta_tags_after_remove = list_finding_tags(self.client, self.namespace, finding.uuid, "meta")
    ...
    def test_finding_structure_analysis(self):
        ...
        meta_fields = [field for field in dir(finding.meta) if not field.startswith('_')]
        print(f"Finding meta fields: {meta_fields}")
        if finding.meta.tags:
            print(f"Finding meta tags: {finding.meta.tags}")
        
        # Analyze finding spec fields
        spec_fields = [field for field in dir(finding.spec) if not field.startswith('_')]
        print(f"Finding spec fields: {spec_fields}")
        if finding.spec.finding_tags:
            print(f"Finding spec finding_tags: {finding.spec.finding_tags}")
        
        # Analyze finding context fields
        context_fields = [field for field in dir(finding.context) if not field.startswith('_')]
    ...
    def test_finding_spec_tag_limitation(self):
        ...
        current_spec_tags = list_finding_tags(self.client, self.namespace, finding.uuid, "spec")
        ...
        updated_finding = add_finding_tag(self.client, self.namespace, finding.uuid, test_spec_tag, "spec")
        
        # Check if tag was added (it shouldn't persist)
        spec_tags_after_add = list_finding_tags(self.client, self.namespace, finding.uuid, "spec")
        ...
        if test_spec_tag not in spec_tags_after_add:
            ...
            print("[WARNING] Spec tags unexpectedly persisted - may indicate API change")
    ...
    def test_finding_operations_summary(self):
        ...
        print(f"  - Update Finding Tags: PATCH /v1/namespaces/{self.namespace}/findings")
...
if __name__ == "__main__":
    ...
    test_instance.findings = findings.list_findings(test_instance.client, test_instance.namespace)
