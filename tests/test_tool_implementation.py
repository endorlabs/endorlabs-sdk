...
class TestToolImplementation:
    ...
    def test_create_namespace_implementation(self, mock_client):
        ...
        with patch('endor_cockpit.resources.namespaces.create_namespace') as mock_create:
            mock_namespace = Mock()
    ...
    def _implement_list_namespaces_tool(self, client: APIClient, tenant_namespace: str, include_children: bool = True) -> List[Any]:
    ...
    def _implement_create_namespace_tool(self, client: APIClient, parent_namespace: str, name: str, description: str, labels: Dict[str, str] = None) -> Any:
    ...
    def _implement_security_scan_tool(self, client: APIClient, target: str, scan_type: str, include_dependencies: bool = True) -> Dict[str, Any]:
...
class TestToolSchemaValidation:
    ...
    def test_tool_schema_completeness(self):
        ...
        for tool_name, schema in tool_schemas.items():
            ...
            if "properties" in params:
                assert isinstance(params["properties"], dict), f"Tool '{tool_name}' properties should be dict"
            
            if "required" in params:
                assert isinstance(params["required"], list), f"Tool '{tool_name}' required should be list"
    ...
    def test_tool_parameter_consistency(self):
        ...
        if "list_namespaces" in tool_schemas:
            ...
            assert "tenant_namespace" in params["properties"], "list_namespaces missing tenant_namespace parameter"
            assert "tenant_namespace" in params["required"], "tenant_namespace should be required"
            
            # Should have include_children parameter
            assert "include_children" in params["properties"], "list_namespaces missing include_children parameter"
        ...
        if "create_namespace" in tool_schemas:
            ...
            for param in required_params:
                assert param in params["properties"], f"create_namespace missing {param} parameter"
