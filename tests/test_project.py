...
from endor_cockpit.resources.projects import Project
from endor_cockpit.resources.tag_management import (
    add_project_tag, remove_project_tag, list_project_tags
)
...
class TestProject:
    ...
    def test_project_get_list(self):
        ...
        for project in projects_list:
            print(f"Project {project.uuid}: {project.meta.name}")
            if project.meta.tags:
                print(f"  Project tags: {project.meta.tags}")
    ...
    def test_project_get_by_uuid(self):
        ...
        retrieved_project = projects.get_project(self.client, self.namespace, project.uuid)
        
        assert retrieved_project is not None, "Should successfully retrieve project by UUID"
        assert retrieved_project.uuid == project.uuid, "Retrieved project should match original"
        assert retrieved_project.meta.name == project.meta.name, "Project name should match"
    ...
    def test_project_structure_analysis(self):
        ...
        meta_fields = [field for field in dir(project.meta) if not field.startswith('_')]
        print(f"Project meta fields: {meta_fields}")
        if project.meta.tags:
            print(f"Project meta tags: {project.meta.tags}")
        
        # Analyze project spec fields
        spec_fields = [field for field in dir(project.spec) if not field.startswith('_')]
        print(f"Project spec fields: {spec_fields}")
        
        # Analyze project tenant_meta fields
        tenant_meta_fields = [field for field in dir(project.tenant_meta) if not field.startswith('_')]
...
if __name__ == "__main__":
    ...
    test_instance.projects = projects.list_projects(test_instance.client, test_instance.namespace)
