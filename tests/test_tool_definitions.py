        ...
        for tool_name, schema in tool_definitions.items():
            if "parameters" in schema and "required" in schema["parameters"]:
                ...
                for param in required_params:
                    assert param in properties, f"Required parameter '{param}' missing from properties in {tool_name}"
...
class TestToolImplementation:
    ...
    def test_list_namespaces_tool(self, mock_client):
        ...
        with patch('endor_cockpit.resources.namespaces.list_namespaces') as mock_list:
            mock_list.return_value = [
                Mock(meta=Mock(name="test-namespace-1", description="Test namespace 1")),
    ...
    def _call_tool(self, tool_name: str, parameters: Dict[str, Any], client: APIClient) -> Any:
...
class TestToolValidation:
    ...
    def test_validate_tool_inputs(self):
        ...
        for tool_name, inputs in valid_inputs.items():
            assert self._validate_inputs(tool_name, inputs), f"Validation failed for {tool_name}"
    ...
    def test_validate_tool_inputs_invalid(self):
        ...
        for tool_name, inputs in invalid_inputs.items():
            assert not self._validate_inputs(tool_name, inputs), f"Validation should fail for {tool_name}"
...
class TestToolIntegration:
    ...
    def _call_tool(self, tool_name: str, parameters: Dict[str, Any], client: Any) -> Any:
        """Simulate calling a tool with given parameters."""
        if tool_name == "list_namespaces":
            if client is None:
...
class TestToolDocumentation:
    ...
    def test_tool_examples(self):
        ...
        for tool_name, example_params in examples.items():
            ...
            assert self._validate_example(tool_name, example_params), f"Invalid example for {tool_name}"
