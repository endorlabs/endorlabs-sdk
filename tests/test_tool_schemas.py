...
This module tests the tool definitions loaded from the
docs/agents/tool-definitions.md file.
"""
    ...
    def test_tool_schema_structure(self):
        ...
        for tool_name, schema in tool_schemas.items():
            ...
            assert "description" in schema, (
                f"Tool '{tool_name}' missing 'description' field"
            )
            assert "parameters" in schema, (
                f"Tool '{tool_name}' missing 'parameters' field"
            )
            ...
            if "properties" in params:
                assert isinstance(params["properties"], dict), (
                    f"Tool '{tool_name}' properties should be a dict"
                )

            if "required" in params:
                assert isinstance(params["required"], list), (
                    f"Tool '{tool_name}' required should be a list"
                )
        ...
        for tool_name, schema in tool_schemas.items():
            ...
            for param_name in required:
                assert param_name in properties, (
                    f"Required parameter '{param_name}' missing from "
...
class TestToolSchemaExamples:
    ...
    def test_tool_parameter_combinations(self):
        ...
        for _tool_name, schema in tool_schemas.items():
            ...
            minimal_params = self._build_minimal_params(properties, required)
            self._validate_example_against_schema(schema, minimal_params)

            # Test with all parameters
            all_params = self._build_all_params(properties)
            self._validate_example_against_schema(schema, all_params)

    def _build_minimal_params(self, properties, required):
        """Build minimal parameter set with only required parameters."""
        minimal_params = {}
    ...
    def _build_all_params(self, properties):
        """Build complete parameter set with all available parameters."""
        all_params = {}
        for param_name, param_def in properties.items():
            if param_def.get("type") == "string":
                ...
                if items_schema.get("type") == "object":
                    all_params[param_name] = [
                        {"action": "test", "condition": "test", "effect": "test"}
                    ]
                else:
    ...
    def _validate_example_against_schema(
        self, schema: Dict[str, Any], example_params: Dict[str, Any]
    ) -> None:
    ...
    def test_tool_schema_consistency(self):
        ...
        for expected_tool in expected_tools:
            if expected_tool in tool_schemas:
                ...
                assert "description" in schema, (
                    f"Tool '{expected_tool}' missing description"
                )
                assert "parameters" in schema, (
                    f"Tool '{expected_tool}' missing parameters"
                )
            ...
            for param_name, param_def in properties.items():
                ...
                if param_def.get("type") == "string":
                    ...
                    assert "description" in param_def, (
                        f"String parameter '{param_name}' in {tool_name} "
                        f"should have description"
                    )

                elif param_def.get("type") == "boolean":
                    # Boolean parameters should have default values
                    assert "default" in param_def, (
                        f"Boolean parameter '{param_name}' in {tool_name} "
                        f"should have default value"
                    )

                elif param_def.get("type") == "array":
                    # Array parameters should specify item types
                    assert "items" in param_def, (
                        f"Array parameter '{param_name}' in {tool_name} "
                        f"should specify item type"
                    )
