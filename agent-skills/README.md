...
Authoring → shipped bundle → runtime mirror flow is documented in [AGENTS.md § Repository layout](../AGENTS.md#repository-layout). **Runtime rule:** agents read the wheel or `.endorlabs-context/sdk/skills/` — not repo `agent-skills/` directly.

After editing skills, run `uv run python devtools/sync_agent_bundle.py` (see [schema/README.md](schema/README.md) for authoring rules). Optional IDE mirrors: `init(sync_skills=...)` or the `endor-context` command below.
...
For **Claude Code** users: copy either `agent-skills/` (portable `SKILL.md` only) or `.cursor/skills/` to `.claude/skills/` at the repo root. The SKILL.md format is identical.
