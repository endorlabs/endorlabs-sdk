...
description: |
  Use when finding potential duplicate Projects across a tenant and child
  namespaces: identical meta.name across namespaces, or names differing only when
  --name-strip-tokens is passed (no default). Excludes SBOM projects (spec.sbom).
  Emits flat CSV and a grouped canvas. Not for findings, scan RCA, or single-project
