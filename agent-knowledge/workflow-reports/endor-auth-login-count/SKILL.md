...
description: |
  Use when aggregating AuthenticationLog login activity by user identity over a
  lookback window (default 90 days) into a CSV (identity, user identifiers, last
  login, login count). Shares helpers with auth troubleshooting. Not for SSO policy
  mapping or per-user auth RCA.
endorlabs:
  catalog:
