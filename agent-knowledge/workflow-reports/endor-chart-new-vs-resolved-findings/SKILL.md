...
description: |
  Use when charting cumulative weekly new vs resolved Critical/High reachable
  vulnerability trends from FindingLog group-by-time queries (CREATE/DELETE via
  FindingLog.list_groups). Default window is the past 90 days with complete weeks
  only; one Cursor canvas per namespace root with optional child traverse. Not for
  open Finding snapshots, PRF/PV resolution reports, or scan pipeline RCA.
endorlabs:
  catalog:
