...
| **`.endorlabs/_cache/sdk/`** | Runtime mirror after `init()` / `endor-context` (gitignored in consumer repos) |
...
Authoring → shipped bundle → runtime mirror flow: [docs/contributing/repository-layout.md](../../docs/contributing/repository-layout.md). **Runtime rule:** agents read the wheel or `.endorlabs/_cache/sdk/skills/` — not repo `agent-knowledge/skills/` directly.
...
To refresh runtime mirrors from `.endorlabs/_cache/sdk/skills/`:
