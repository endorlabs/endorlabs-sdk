...
summary: >-
  Place maintainer automation in `devtools/precommit|codegen|ship` or
  `endorlabs.context` hooks; share git staged utilities; keep
  `.pre-commit-config.yaml` as wiring only; one home per guard.
---
...
| Commit guards | [`devtools/precommit/pre_commit_guards.py`](../../devtools/precommit/pre_commit_guards.py) | Block `.env` / `.endorlabs/`; **Unreleased** reminder (stderr, exit 0); **fail** on emails / non-Endor URLs / estate `-n` flags (`external-pii-urls`) and estate literals (`portable-examples`) on staged checked-in paths |
| Context refresh | [`src/endorlabs/context/_maintainer_refresh.py`](../../src/endorlabs/context/_maintainer_refresh.py) | Pre-commit bundle sync + local `.endorlabs/` refresh (paths via hook argv) |
...
3. **Thin hooks** — new pre-commit checks: add a subcommand to `precommit/pre_commit_guards.py` or a sibling under `devtools/precommit/`; wire with `uv run python devtools/precommit/...`; add unit tests under `tests/unit/tooling/scripts/`. Put generators under `codegen/`, release gates under `ship/`. Do not add regen scripts to `.github/scripts/`.
...
1. Implement the check in `devtools/precommit/pre_commit_guards.py` (or extend `git_staged.py` when only listing/normalizing paths).
