...
| ------------ | ----------------------------------- |
| **blocked-paths** | Refuses staging `.env` or anything under `.endorlabs/` (session dumps, live OpenAPI, workspace artifacts). Put secrets and tenant dumps only in gitignored paths. |
...
| **context-root-literals** | Block hard-coded `.endorlabs/...` path constants in SDK / skill scripts — use path helpers. |
