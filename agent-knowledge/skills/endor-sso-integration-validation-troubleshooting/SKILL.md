...
```bash
uv run --env-file .env python agent-knowledge/skills/endor-sso-integration-validation-troubleshooting/sso_access_spotcheck.py --tenant-hint root --output-dir .endorlabs-context/workspace/sessions/agent/exports
```

Then optionally narrow by actor:

```bash
uv run --env-file .env python agent-knowledge/skills/endor-sso-integration-validation-troubleshooting/sso_access_spotcheck.py --tenant-hint root --target-email user@example.com --target-group group-a --target-group group-b --output-dir .endorlabs-context/workspace/sessions/agent/exports
```
