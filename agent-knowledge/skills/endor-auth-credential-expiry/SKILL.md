...
description: >-
  Audit tenant APIKey resources for expired or soon-to-expire credentials.
  Lists API keys on a namespace path, classifies expiration status, and writes
  a CSV report (default 30-day lookahead). Use for tenant credential hygiene
  and rotation planning—not for bearer ENDOR_TOKEN refresh, SSO policy mapping,
  or AuthenticationLog RCA.
endorlabs:
  catalog:
