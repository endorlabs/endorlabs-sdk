...
description: >-
  Audit AuthorizationPolicy wire shape and claim/namespace form for common
  misconfigurations (comma-separated namespace blobs, wrong IdP claim keys,
  double provider suffixes). Use when reviewing Auth Policy setup, "no
  authorized tenant" after successful IdP login, or customer policy imports—
  not for credential refresh (endor-auth-setup), Enterprise SSO planning
  (endor-sso-integration-validation-troubleshooting), or AuthenticationLog RCA
...
Authoring path (this repo): `agent-knowledge/skills/endor-audit-authorization-policies/scripts/…`.
