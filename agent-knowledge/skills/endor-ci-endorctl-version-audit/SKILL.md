...
description: Tenant-wide audit of projects whose latest scan within a recent window
  ran via CLI endorctl (not agentless Cloud scans). Aggregates project counts by
  endorctl version from ScanResult.spec.environment.endorctl_version. Use when
  checking CI endorctl adoption, stale CLI versions, or version-specific rollout
  status—not for scan pipeline RCA or Cloud vs CLI registration classification alone.
---
...
```bash
uv run python agent-knowledge/skills/endor-ci-endorctl-version-audit/scripts/audit_ci_endorctl_versions.py \
  --tenant <tenant> \
