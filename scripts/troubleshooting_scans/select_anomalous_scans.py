...
try:
    from scripts.troubleshooting_scans.common import load_json, write_json
...
def anomaly_score(
    ...
    findings_current = sum(
        int(current.get(k, 0))
        for k in ("findings_critical", "findings_high", "findings_medium", "findings_low")
    )
    ...
    if int(current.get("scan_success", 0)) == 0 and int(previous.get("scan_success", 0)) > 0:
        score += 4
