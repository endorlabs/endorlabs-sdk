...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Pull full logs for a scan result UUID.")
...
def _select_project(client: endorlabs.Client, tenant: str, query: str) -> Any:
    ...
    for project in projects[:80]:
        ...
        if scans:
            return project
    raise ValueError("No eligible project with scan results found for this tenant/query.")
...
def _dedupe_key(message: Any) -> tuple[str | None, str]:
    ...
    payload_s = json.dumps(payload, sort_keys=True, ensure_ascii=True) if payload else ""
