...
- Manual policy overrides belong in:
  ...
  - `scripts/model_sync_profiles/*.json` (policy/profile metadata),
...
- `registry_parity_report.status=fail`:
  - check `mapping/missing_in_mapping`, then `scripts/sync/policy.py` aliases.
...
- stub description validation failures:
  - check `scripts/model_sync_profiles/resource_descriptions.json`.
