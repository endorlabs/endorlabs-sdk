...
def _run_optional_generators(
    ...
    ok = True
    if generate_stubs:
        result = _run_command([sys.executable, "scripts/generate_client_stub.py"], REPO_ROOT)
    ...
    if generate_reference_docs:
        result = _run_command([sys.executable, "scripts/generate_reference_docs.py"], REPO_ROOT)
        if result.returncode != 0:
            ok = False
            logger.error(
