...
def match_producer_projects(
    ...
    if k in produced_by and produced_by[k]:
        s = produced_by[k]
        amb = len(s) > 1
    ...
    for fr, to, sp in supporting:
        ...
        if k4 not in best:
            ...
            if (
                ...
                and o.evidence_tier != "tier_a_exact"
            ):
                best[k4] = sp
...
def bfs_min_path(
    ...
    *num_hops* is number of edges. Requires num_hops >= 2 to count as an indirect
    ...
    q: deque[tuple[str, list[str], list[str], int]] = deque(
        [(start, [start], [], 0)]
    )
    while q:
...
def build_adj_and_edge_tiers(
    ...
    for e in edges:
        ...
        u_s, w_s = str(u), str(w)
        tv: str = str(
            e.get("evidence_tier") or "tier_b_name_only"
        )
        k = (u_s, w_s)
        if k not in best:
            best[k] = tv
    ...
    for src in project_uuids:
        for tgt in project_uuids:
            if src == tgt or (src, tgt) in seen:
                continue
            path, e_t, nh = bfs_min_path(
                str(src), str(tgt), adj, edge_t, max_hops
            )
