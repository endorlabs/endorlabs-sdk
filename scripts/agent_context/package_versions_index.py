...
def index_row_from_pv(pv: Any, *, project_uuid: str, namespace: str) -> dict[str, Any]:
    ...
    create_t = (
        _iso_timestamp(getattr(meta, "create_time", None)) if meta else None
    )
    update_t = (
        _iso_timestamp(getattr(meta, "update_time", None)) if meta else None
    )
    upsert_t = (
        _iso_timestamp(getattr(meta, "upsert_time", None)) if meta else None
    )
    return {
...
def select_top_n_uuids_by_update_time(
