...
| Publisher index / PV sweep | [`dependency_graph.py`](src/endorlabs/workflows/relationships/dependency_graph.py) `build_publisher_index` | One `PackageVersion.list(traverse=True)` | — | Traverse concurrent | Large single stream; optional future: PV list per `project_uuid` if benchmark wins |
...
**Primitives to reuse:** [`parallel_map_shards()`](src/endorlabs/workflows/sharded_collect.py), [`list_resource_count()`](src/endorlabs/workflows/list_bounds.py), [`format_progress()`](src/endorlabs/workflows/list_bounds.py), [`execute_across_namespaces()`](src/endorlabs/utils/parallel.py), [`main_context_filter()`](src/endorlabs/workflows/relationships/context_filters.py) for DM/PV main-context rows. Deep dive: [relationships/README.md](src/endorlabs/workflows/relationships/README.md).
