"""Call graph summary and analysis artifact helpers.

Wire decode stays in ``endorlabs.resources.call_graph_data_proto`` /
``CallGraphData`` facade. This module formats decoded envelopes for hydration.
"""
...
def generate_call_graph_analysis_md(cg_json_path: str | Path) -> str | None:
    ...
    if "zstd_bytes" not in envelope:
        return None
    try:
        # Lazy: analysis markdown lives in workflows.callgraph.render.
