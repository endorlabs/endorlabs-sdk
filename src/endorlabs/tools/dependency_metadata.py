...
Project-scoped fetch uses ``spec.importer_data.project_uuid`` (no
``DependencyMetadata.list_by_project`` route). Prefer
``DependencyMetadata.list_for_context`` when a ScanResult is available.
"""
...
from endorlabs.tools.list_bounds import is_list_truncated, resolve_max_pages
