...
| **3 — Call-graph export** | Optional; enumerates PVs and exports call graph payloads | `callgraph_export/` + nested manifest | `--callgraph-max-pages`, `--callgraph-page-size` |
...
- **`artifacts.callgraph_export`**: Pass 3; `null` if not run. Object includes `pass`, `package_versions_total`, `call_graph_exports_total`, list caps.
...
1. **Before** claiming “all package versions” or “no call graph,” read `warnings` and `inventory.truncated` / `hydration` / `callgraph_export` caps.
...
- “Full call-graph exports for listed PVs” → `--callgraph-export` (Pass 3), optionally `--decode-zstd`.
...
Pass 3:  --callgraph-export, --callgraph-max-pages, --callgraph-page-size, --decode-zstd
