...
description: Generates a tenant-wide PRF (potentially reachable function) vulnerability
  and PackageVersion resolution error report for main-context NuGet, NPM, Maven, and
...
```bash
uv run python sdk/skills/endor-potentially-reachable-analysis/scripts/run_report.py \
  <tenant> \
