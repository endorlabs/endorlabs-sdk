...
description: Audit tenant APIKey resources for expired or soon-to-expire credentials.
  Lists API keys on a namespace path, classifies expiration status, and writes a CSV
  report (default 30-day lookahead). Use for tenant credential hygiene and rotation
  planning—not for bearer ENDOR_TOKEN refresh, SSO policy mapping, or AuthenticationLog
  RCA.
---
...
Run from repo authoring path or from `.endorlabs-context/sdk/skills/endor-auth-credential-expiry/scripts/`
after `init()`.

```bash
uv run --env-file .env python .endorlabs-context/sdk/skills/endor-auth-credential-expiry/scripts/credential_expiry_report.py \
  --tenant <tenant> \
  --within-days 30 \
  --platform-wide
```
