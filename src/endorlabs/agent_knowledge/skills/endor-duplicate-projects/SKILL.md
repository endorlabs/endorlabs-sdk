...
description: 'Find potential duplicate Projects across a tenant and all child namespaces:
  ...
  tokens. Excludes SBOM projects (spec.sbom). Emit a flat CSV (project name, namespace,
  uuid, source) and a grouped canvas. Use when auditing project inventory or deduplicating
  registrations—not for findings, scan RCA, or single-project classification alone.'
---
