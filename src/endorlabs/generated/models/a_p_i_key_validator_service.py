...
#   filename:  a_p_i_key_validator_service.json
...
class V1ValidateAPIKeyReq(BaseModel):
    issuing_user: V1User | None = Field(
        None, description='The user that creates the API key.'
    )
    key: str = Field(..., description='The unique identifier of the API key.')
    secret: str = Field(
        ..., description='The secret associated with the provided API Key.'
    )
    valid: bool | None = Field(
        None, description='Valid returns if the API key and secret pair are valid.'
    )
