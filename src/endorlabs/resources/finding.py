...
def _get_finding_ops(client: APIClient) -> BaseResourceOperations[Finding]:
...
def get_finding(
    ...
    Raises:
        NotFoundError: If finding doesn't exist
        PermissionDeniedError: If user lacks permission
        ServerError: If server error occurs

    """
    ops = _get_finding_ops(client)
    return ops.get(tenant_meta_namespace, finding_uuid)


def create_finding(
    client: APIClient, tenant_meta_namespace: str, payload: CreateFindingPayload
) -> Finding:
    """Create a new finding with pre-validation and typed errors.
    ...
    Raises:
        ...
        PermissionDeniedError: If user lacks permission
        ConflictError: If finding already exists
        ServerError: If server error occurs
...
def update_finding(
    ...
    try:
        ...
        # Merge current finding with payload updates
        # Ensure tags are properly merged (not None)
        current_tags = current_finding.meta.tags or []
        payload_tags = payload.meta.tags if payload.meta and payload.meta.tags else None

        merged_meta = {
            "name": current_finding.meta.name,  # Required field
            "tags": payload_tags if payload_tags is not None else current_tags,
            **(
                {
                    k: v
                    for k, v in payload.meta.model_dump(exclude_none=True).items()
                    if k != "tags"  # tags handled separately above
                }
                if payload.meta
                else {}
            ),
        ...
        # Ensure finding_tags are properly merged
        current_finding_tags = current_finding.spec.finding_tags or []
        payload_finding_tags = (
            payload.spec.finding_tags
            if payload.spec and payload.spec.finding_tags
            else None
        )
        ...
        merged_spec = {
            **current_finding.spec.model_dump(),  # Include all existing spec fields
            "finding_tags": (
                payload_finding_tags
                if payload_finding_tags is not None
                else current_finding_tags
            ),
            **(
                {
                    k: v
                    for k, v in payload.spec.model_dump(exclude_none=True).items()
                    if k != "finding_tags"  # finding_tags handled separately above
                }
                if payload.spec
                else {}
            ),
        ...
        merged_context = (
            current_finding.context.model_dump() if current_finding.context else {}
        )

        # Build merged finding object for base class
        tenant_meta_dict = (
            current_finding.tenant_meta.model_dump()
            if current_finding.tenant_meta
        ...
        # Create Finding object from merged data
...
def delete_finding(
    ...
    """
    ops = _get_finding_ops(client)
    return ops.delete(tenant_meta_namespace, finding_uuid)
