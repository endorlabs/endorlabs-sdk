...
class CustomFacadeEntry:
    ...
    ``pyi_*`` fields are consumed only by
    ``devtools/codegen/generate_client_stub.py`` so
...
def _load_generated_runtime_contract() -> list[dict[str, Any]]:
    try:
        ...
        raise RuntimeError(
            ...
            "Run: uv run python devtools/codegen/model_sync.py"
        ) from error
...
CUSTOM_FACADE_REGISTRY: list[CustomFacadeEntry] = [
    ...
    CustomFacadeEntry(
        ...
        pyi_import_module="facade",
        pyi_attr_doc=(
            "Kind-agnostic Query graph joins via execute / at_namespace / create. "
            "Estate recipes: client.Query.Project.count_pv / "
            "count_findings_by_category / count_dm; see docs/guides/query-recipes.md."
        ),
    ),
]
