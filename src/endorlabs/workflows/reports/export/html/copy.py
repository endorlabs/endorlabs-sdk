...
<div class="glossary">
  ...
  <ul>
    ...
    <li><strong>SCA burndown</strong> filters vulnerability FindingLog events by
      ...
      function reachability (any reachability default; RF+PRF / RF / PRF /

      unreachable function). Dependency reach is omitted because function reach

      implies a reachable dependency. Severity uses cumulative
...
def packet_readme(
    *,
    ...
    """Return README.txt body for a packet (or patches-only) render."""
    if patches_only:
        return README_PATCHES_ONLY_TEXT
    skip: set[str] = set()
    if not include_sast:
        skip.update(
            {
    ...
    if not skip:
        return README_TEXT
    lines = [ln for ln in README_TEXT.splitlines() if ln not in skip]
    out: list[str] = []
    blank = 0
    for ln in lines:
        if ln == "":
