...
def setup_logging(module_name: str = "endor_cockpit") -> logging.Logger:
    ...
    try:
        with open("pyproject.toml", "rb") as f:
            ...
            level = os.getenv("LOG_LEVEL") or module_config.get("level", log_config.get("level", "INFO"))
            format_str = module_config.get("format", log_config.get("format", "%(levelname)s - %(message)s"))
