...
from .finding import Finding, get_finding
...
def add_project_tag(
    client: APIClient, namespace: str, project_uuid: str, tag: str
) -> Optional[Project]:
    """
...
def remove_project_tag(
    ...
    try:
        ...
        logger.error(
            f"Error removing tag from project {project_uuid}: {e}", exc_info=True
        )
        return None


def _get_finding_tag_info(finding: Finding, tag_type: str) -> tuple[str, list]:
    """Get field path and current tags for a finding based on tag type."""
    if tag_type == "meta":
        return "meta.tags", getattr(finding.meta, "tags", []) or []
    elif tag_type == "spec":
        return "spec.finding_tags", getattr(finding.spec, "finding_tags", []) or []
    elif tag_type == "context":
        return "context.tags", getattr(finding.context, "tags", []) or []
    else:
        raise ValueError(
            f"Invalid tag_type: {tag_type}. Must be 'meta', 'spec', or 'context'"
        )


def _build_finding_request_data(
    finding: Finding, finding_uuid: str, namespace: str, tag_type: str, new_tags: list
) -> dict:
    """Build request data for finding tag update."""
    request_data = {
        "request": {
            "update_mask": (
                f"{tag_type}.tags"
                if tag_type == "meta"
                else f"{tag_type}.finding_tags"
                if tag_type == "spec"
                else f"{tag_type}.tags"
            )
        },
        "object": {"uuid": finding_uuid, "tenant_meta": {"namespace": namespace}},
    }
    ...
    if tag_type == "meta":
        ...
        request_data["object"]["spec"] = {
            ...
            "level": (
                str(finding.spec.level.value)
                if hasattr(finding.spec.level, "value")
                else str(finding.spec.level)
            ),
...
def add_finding_tag(
    ...
    try:
        ...
        # Get field path and current tags
        try:
            field_path, current_tags = _get_finding_tag_info(finding, tag_type)
        ...
        # Build request data
        request_data = _build_finding_request_data(
            finding, finding_uuid, namespace, tag_type, new_tags
        )
...
def remove_finding_tag(
    ...
    try:
        ...
        logger.error(
            f"Error removing tag from finding {finding_uuid}: {e}", exc_info=True
        )
        return None
...
def list_project_tags(
    ...
    try:
        ...
        logger.error(
            f"Error listing tags for project {project_uuid}: {e}", exc_info=True
        )
        return []


def list_finding_tags(
    client: APIClient, namespace: str, finding_uuid: str, tag_type: str = "all"
) -> Union[List[str], dict]:
    """
    ...
    try:
        ...
        logger.error(
            f"Error listing tags for finding {finding_uuid}: {e}", exc_info=True
        )
        return []


def bulk_tag_projects(
    client: APIClient, namespace: str, project_uuids: List[str], tag: str
