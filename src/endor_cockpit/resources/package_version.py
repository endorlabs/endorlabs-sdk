...
class Bom(BaseModel):
    ...
    dependencies: list[Union[PackageVersionDependency, dict[str, Any]]] | None = Field(
...
class PackageVersionSpec(BaseSpec):
    ...
    resolved_dependencies: Union[Bom, dict[str, Any]] | None = Field(
    ...
    precomputed_call_graph_state: (
        Union[PrecomputedState, str, dict[str, Any]] | None
    ) = Field(
        None,
        description=(
    ...
    def validate_source_code_reference(cls, v: Any) -> Any:
        ...
        if isinstance(v, dict):
            # If version is nested, extract ref and sha to top level
