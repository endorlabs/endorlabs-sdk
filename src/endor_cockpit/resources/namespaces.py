"""
This module provides a resource-oriented interface for managing Endor Labs namespaces.
It implements CRUD operations following REST principles and provides type-safe data models.
"""
...
def list_namespaces(client: APIClient, tenant_namespace: str) -> List[Namespace]:
    ...
    try:
        ...
        res = client.get(f'v1/namespaces/{tenant_namespace}/namespaces', headers=headers)
...
def update_namespace(client: APIClient, parent_namespace: str, namespace_uuid: str, payload: UpdateNamespacePayload) -> Optional[Namespace]:
    ...
    try:
        ...
        res = client.patch(f'v1/namespaces/{parent_namespace}/namespaces/{namespace_uuid}', headers=headers, data=payload.model_dump())
        data = res.json()
        return Namespace(**data)
    except Exception as e:
...
if __name__ == "__main__":
    ...
    spec_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'tmp', 'openapiv2.swagger.json')
    try:
    ...
    mock_namespaces_to_create = [
        CreateNamespacePayload(meta=NamespaceMetaCreate(name=f"mock-namespace-{i}", description=f"Description for mock-namespace-{i}"))
        for i in range(3)
    ]
