...
def analyze_openapi_services(file_path: Path) -> List[SectionMetrics]:
    ...
    try:
        ...
        for path, methods in paths.items():
            ...
            service_groups[service_name].append({"path": path, "methods": methods})
...
def analyze_category(
    category_name: str, file_patterns: List[str], analyzer_func, base_path: Path = None
) -> CategoryAnalysis:
    ...
    if token_stats["max"] > 5000:
        recommendations.append(
            f"WARNING: Some sections exceed 5000 tokens (max: {token_stats['max']})"
        )

    if token_stats["p95"] > 3000:
        recommendations.append(
            f"Consider chunk size >= {token_stats['p95']} to preserve 95% of sections"
        )
    else:
        recommendations.append(
            f"Recommended chunk size: {token_stats['p95']} tokens (P95)"
        )

    if token_stats["median"] < 1000:
        recommendations.append(
            f"Many small sections (median: {token_stats['median']} tokens) - good for granular retrieval"
        )
...
def main():
    ...
    for analysis in results:
        ...
        print(f"  Files analyzed: {analysis.files_analyzed}")
        print(f"  Sections found: {analysis.sections_found}")
        print("  Size distribution (tokens):")
    ...
    print("Recommended chunk sizes based on analysis:")
    ...
    print("  - Hard limit: 5000 tokens (for extremely large sections)")
    print()
